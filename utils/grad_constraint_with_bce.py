...
class GradConstraint_BCE:
    ...
    def __init__(self, model, modules, channel_paths):
        ...
        for channel_path in channel_paths:
            self.channels.append(torch.from_numpy(np.load(channel_path)))

    def loss_channel(self, outputs, labels, batch):
        # high response channel loss
        # probs = torch.argsort(-outputs, dim=1)
        probs = torch.zeros([outputs.shape[0],2], dtype=labels.dtype).to(labels.device)
        probs[outputs.squeeze(1)>0, 0] = 1
        probs[outputs.squeeze(1)<=0,1] = 1
        # labels_ = []
        ...
        nll_loss_ = torch.nn.BCEWithLogitsLoss()(outputs, labels_.float().unsqueeze(1))

        # low response channel loss
        nll_loss = torch.nn.BCEWithLogitsLoss()(outputs, labels.float().unsqueeze(1))
